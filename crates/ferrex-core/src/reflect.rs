...
pub fn detect_contradictions(
    ...
    for mems in by_subject.values() {
        ...
        for i in 0..mems.len() {
            for j in (i + 1)..mems.len() {
                // Skip if both already matched elsewhere — best-effort dedup to reduce noise.
                // A memory can still appear in multiple contradiction pairs via different mechanisms.

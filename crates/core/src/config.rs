...
fn resolve_staleness(file: &StalenessFile, defaults: &StalenessConfig) -> StalenessConfig {
    ...
    let overrides: [(ferrex_store::MemoryType, &Option<TypeStalenessFile>); 3] = [
        (ferrex_store::MemoryType::Episodic, &file.episodic),
        (ferrex_store::MemoryType::Semantic, &file.semantic),
        (ferrex_store::MemoryType::Procedural, &file.procedural),
    ];
    for (mem_type, file_override) in overrides {
        if let Some(ovr) = file_override {
            let entry = type_config
                .get_mut(&mem_type)
                .expect("default type_config must contain all MemoryType variants");
            apply_type_override(entry, ovr);
        }
    }
...
const fn apply_type_override(entry: &mut TypeStalenessConfig, ovr: &TypeStalenessFile) {
    if let Some(v) = ovr.half_life_days {
        entry.half_life_days = v;
    }
    if let Some(v) = ovr.stale_threshold {
